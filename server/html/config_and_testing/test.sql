...
/*for following*/
select users.userID, users.username, borks.bork from users inner join borks on users.userID=borks.userID where users.userID in (
        select user2_ID from following where user1_ID=4
);

/*for discover*/
select users.userID, users.username, borks.bork from users inner join borks on users.userID=borks.userID where users.userID!=4 and users.userID not in (
        select user2_ID from following where user1_ID=4
);
